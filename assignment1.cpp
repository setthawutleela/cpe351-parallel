    ...
    if (p == 1) {
        ...
        while (loop < atoi(argv[3])) {
            for (i = 1; i < p; i++) {
                ...
                MPI_Isend(&(pMat[rowPos]), (groupRows + 2) * ncols, MPI_FLOAT, i, 0, MPI_COMM_WORLD, &sRequest);
        ...
        int loop = 0;
        while (loop < atoi(argv[3])) {
            MPI_Irecv(&(pMatBuf[0]), (groupRows + 2) * ncols, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, &rRequest[rank]);
